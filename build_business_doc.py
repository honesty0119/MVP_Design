...
add_bullets(doc, [
    ...
    "线索分析 Agent：通过 OpenAI-compatible API 让 LLM 选择“渠道转化率、超时线索、漏斗快照”三个受控工具；工具结果回填后再生成回答。无密钥或模型故障时明确回退规则查询。",
])
...
add_p(doc, "验证证据：自动化测试覆盖手机号去重、非法跨级、无效原因必填、外呼回调幂等、LLM 选择受控工具并完成结果回填、无密钥规则回退，以及新线索→待外呼→回调接通→有效→跟进→MQL→SQL 的完整业务链路。")
