...
OUT = os.getenv("BUSINESS_DOC_OUTPUT", "业务设计文档.docx")
BLUE = "315EFB"
NAVY = "162033"
MUTED = "667085"
LIGHT = "F2F4F7"
...
add_bullets(doc, [
    ...
    "负责人代表当前承接责任，不代表数据所有权。Demo 提供轻量角色模拟并在服务端校验销售/主管边界；生产系统仍需接入真实登录、团队数据范围和审批流。",
...
add_table(doc, ["阶段", "业务动作", "进入/退出条件", "异常与责任"], [
    ...
    ("MQL / SQL", "按准入条件完成转化", "MQL 至少有负责人和跟进，并展示评分解释；SQL 需商机说明", "无效需原因；跨级状态被服务端拒绝"),
], [0.8, 1.55, 2.25, 1.9])
...
add_table(doc, ["对象", "核心字段", "设计理由 / 边界"], [
    ("Lead", "phone、source、channel、status、owner_id、created_at", "当前快照；手机号唯一仅是 MVP 假设；评分由状态、跟进、渠道等实时计算"),
    ("StatusHistory", "from_status、to_status、changed_at、note", "状态变化独立留痕；用于解释阶段路径"),
    ...
    ("AuditLog", "lead_id、actor_id、action、detail、created_at", "关键写操作留痕；不是用前端按钮隐藏来代替权限"),
], [1.05, 2.7, 2.75])
...
add_p(doc, "评分解释：MVP 额外计算 0–100 分的线索评分，拆为联系方式有效性、责任归属、跟进事实、渠道意向、私域承接和跟进时效。评分用于解释“为什么建议/不建议转 MQL”，不替代硬性状态机；这样既能体现业务判断，又避免模型或前端随意改漏斗口径。")

add_h(doc, "5. MVP 页面与操作")
add_bullets(doc, [
    "线索列表：按状态、来源、负责人筛选；同时呈现当前负责人、最近跟进和超时标记。",
    "线索详情：负责人分配、跟进追加、线索评分、MQL 阻塞项、下一合法状态、无效/商机原因、状态历史和审计记录。",
    "漏斗看板：总线索、有效率、MQL/SQL 转化率、超 48 小时未跟进数。",
    "新增/导入：单条新增和 CSV 导入均可页面演示；单次限 500 条并返回重复数与逐行错误。",
    "权限演示：页面顶部可切换当前操作人；销售只能操作自己负责的线索，主管可转移负责人和复核无效线索。",
    "线索分析 Agent：通过 OpenAI-compatible API 让 LLM 选择“渠道转化率、超时线索、漏斗快照”三个受控工具；页面展示工具、参数、服务端校验和结果回填链路。",
])
...
add_p(doc, "自然语言查询采用 LLM → Tool Calling → 参数校验 → 预定义参数化查询 → 工具结果回填 → 最终回答的 Agent 循环。模型没有表名、字段名或 SQL 字符串参数，无法执行任意 SQL；API Key 仅从环境变量读取。若无密钥或模型网络失败，系统回退到确定性规则并在页面标注真实执行模式。")

doc.add_page_break()
add_h(doc, "8. 权限、隐私与审计边界")
add_bullets(doc, [
    "销售：Demo 中只能操作自己负责的线索；生产中还应限制列表查询范围、批量导出和跨团队数据。",
    "主管：可查看团队、分配/回收负责人、复核无效重开；关键操作必须记录操作人、前后值和原因。",
...
add_h(doc, "10. 现场追问准备")
add_table(doc, ["追问", "回答边界"], [
    ("100 条无效线索如何定位？", "先按来源/渠道/落地页版本和无效原因聚合，再叠加投放成本；不能只看线索量。"),
    ("同手机号多渠道重复进线？", "Demo 保留首次归因；生产拆客户主体和线索记录，按归因窗口与业务线确定首触/末触规则。"),
    ("Agent 为什么不用任意 Text-to-SQL？", "指标口径必须稳定；Agent 只选受控工具，服务端执行参数化查询并展示数据边界。"),
    ("MVP 如何扩展？", "先补身份权限和可靠回调，再补 cohort 漏斗、阶段耗时、渠道成本和归因模型。"),
], [2.0, 4.5])

add_h(doc, "11. MVP 取舍与迭代")
add_p(doc, "本次选择 Python 标准库 + SQLite + 原生页面，避免依赖安装阻塞演示，把有限时间用在状态机、评分解释、权限边界、幂等、历史留痕和指标口径上。未实现真实登录、复杂 RBAC、真实三方连接和异步任务，均已明确为生产化边界，而非默认“前端可信”。")
add_p(doc, "验证证据：自动化测试覆盖手机号去重、非法跨级、无效原因必填、销售越权拒绝、线索评分解释、外呼回调幂等、LLM 选择受控工具并完成结果回填、无密钥规则回退，以及新线索→待外呼→回调接通→有效→跟进→MQL→SQL 的完整业务链路。")
add_table(doc, ["优先级", "下一步", "验收信号"], [
    ("P0", "真实登录、团队数据范围、审批流、手机号脱敏", "越权测试通过；导出可审批；敏感字段按角色展示"),
